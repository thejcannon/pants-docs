...
Pants uses [`docker_image`](doc:reference-docker_image) [targets](../using-pants/concepts/targets.md) to indicate which Dockerfiles you want Pants to know about, and to add any necessary metadata.

You can generate initial BUILD files for your Docker images, using [tailor](../getting-started/initial-configuration.md#5-generate-build-files):
...
!!! warning "The `docker_image` `instructions` field"
...
- Loose files specified using [`file` / `files` targets](../using-pants/assets.md#files).
...
See [here](tagging-docker-images.md) for how to set up registries.
