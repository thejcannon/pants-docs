...
Then, you partition your requirements targets across these resolves using the `resolve` field, and possibly the [parametrize](../../using-pants/concepts/targets.md#parametrizing-targets) mechanism:
...
If a first-party target is compatible with multiple resolves, e.g., shared utility code, you can use the [parametrize](../../using-pants/concepts/targets.md#parametrizing-targets) mechanism with the `resolve=` field.
...
In theory, when you generate a lockfile, you should want to audit it for bugs, compliance and security concerns. In practice this is intractable to do manually. We would like to integrate with automated auditing tools and services in the future, so watch this space for updates, or feel free to [reach out on Slack](../../getting-help/the-pants-community/index.md) if this is important to you and you'd like to work on it.
...
```Text title="3rdparty/python/pytest-requirements.txt"
...
!!! warning "The previous way of generating tool lockfiles is deprecated!"
