...
!!! warning "Java and Scala support is beta stage"
    ...
    Please share feedback for what you need to use Pants with your JVM project by either [opening a GitHub issue](https://github.com/pantsbuild/pants/issues/new/choose) or [joining our Slack](../getting-help/the-pants-community/index.md)!
...
To override the default on a particular target, you can use the [`jdk=` field](doc:reference-java_source#codejdkcode). It can be useful to use the [`parametrize` builtin](../using-pants/concepts/targets.md#parametrizing-targets) with the `jdk=` field, particularly to run test targets under multiple JDKs.
...
To cross-build a set of Scala targets for multiple Scala versions, you can use the [`parametrize` builtin](../using-pants/concepts/targets.md#parametrizing-targets) with the `resolve=` field of the target and its dependencies.

!!! warning "A jvm_artifact for scala-library artifact is explicitly required."
