...
In this tutorial, we continue from where we've left in the [previous tutorial](create-a-new-goal.md). Having now a complete goal with a custom target, we are ready to make certain improvements and learn more advanced concepts that you would likely find useful when working on your own plugins.
