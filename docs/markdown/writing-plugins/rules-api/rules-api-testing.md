...
You must also install the distribution `pantsbuild.pants.testutil`. We recommend using the [`pants_requirements` target to do this](../index.md).
...
!!! warning "Confusing rule graph error?"
    ...
    Please feel free to reach out on [Slack](../../getting-help/the-pants-community/index.md) for help with figuring out how to get things working.
...
`rule_runner.request()` is equivalent to how you would normally use `await Get(MyOuput, Input1, input1_instance)` in a rule (See [Concepts](rules-api-concepts.md)). For example, if you would normally say `await Get(Digest, MergeDigests([digest1, digest2])`, you'd instead say `rule_runner.request(Digest, [MergeDigests([digest1, digest2])`.
...
Note that our `@rule` takes 3 parameters, but we only explicitly included `BanditRequest` in the inputs. This is possible because the engine knows how to compute all [Subsystems](rules-api-subsystems.md) based on the initial input to the graph. See [Concepts](rules-api-concepts.md).

We are happy [to help](../../getting-help/the-pants-community/index.md) figure out what rules to register, and what inputs to pass to `rule_runner.request()`. It can also help to [visualize the rule graph](rules-api-tips.md) when running your code in production. If you're missing an input that you need, the engine will error explaining that there is no way to compute your `OutputType`.
