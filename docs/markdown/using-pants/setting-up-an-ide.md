...
To get your editor to understand the repo's first-party sources, you will probably need to tell it about the repo's [source roots](concepts/source-roots.md). You can list those with:
...
Assuming you are using the ["resolves" feature for Python lockfiles](../python/python-overview/python-third-party-dependencies.md)—which we strongly recommend—Pants can export a virtualenv for each of your resolves. You can then point your IDE to whichever resolve you want to load at the time.
...
You can specify the `--resolve` flag [multiple times](concepts/options.md#list-values) to export multiple virtualenvs at once.
...
If you're using [Protobuf and gRPC](../python/python-integrations/protobuf-python.md), you may want your editor to be able to index and navigate the generated source code.
...
See the following links for instructions on how to do so under the [test goal](../python/python-goals/python-test-goal.md) and under the [run goal](../python/python-goals/python-run-goal.md).

## IDE integrations

We have not yet developed tight IDE integrations, such as a PyCharm plugin or a VSCode extension, that would allow the IDE to run Pants on your behalf. If you're interested in developing this functionality for your favorite IDE, [let us know](../getting-help/the-pants-community/index.md)!
